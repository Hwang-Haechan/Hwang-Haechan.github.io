...
# port-forwarding-and-ddns-setup

A guide to enabling SSH access from anywhere — including mobile data networks — using router port forwarding and DDNS.

## Background

When your server runs inside a VM on a home network, its IP address (e.g. `192.168.x.x`) is a private address only reachable from within the same Wi-Fi network. To access it from outside (mobile data, a different network, etc.), you need to set up port forwarding through your router and optionally DDNS to handle dynamic public IPs.
...
External (mobile data / different network)
       ↓
Public IP (e.g. 203.0.113.1) : Port 2222
       ↓
Router Port Forwarding
       ↓
Laptop (192.168.1.100) : Port 2222
       ↓
VMware NAT Port Forwarding
       ↓
Ubuntu VM Server (192.168.100.250) : Port 22
```
...
- Add a port forwarding rule:
  - **Host Port**: `2222`
  - **Type**: `TCP`
  - **Virtual Machine IP**: `192.168.100.250`
...
| Protocol | TCP |
| Internal IP Address | `192.168.1.100` (laptop's local IP) |
| Internal Port | `2222` |
...
2. Enter a subdomain name (e.g. `your-server`) and click **add domain**.
3. Your domain will be: `your-server.duckdns.org`
...
| Host Domain | `your-server` (subdomain only, without `.duckdns.org`) |
...
```
your-server    203.0.113.1    updated X minutes ago
```
...
```bash
ssh username@your-server.duckdns.org -p 2222
```

Or with a key file specified explicitly:

```bash
ssh -i ~/.ssh/id_ed25519 username@your-server.duckdns.org -p 2222
```

For mobile SSH clients (e.g. Terminus on iOS), update the host settings:

- **Host**: `your-server.duckdns.org`
- **Port**: `2222`
...
- Home internet connections typically use a dynamic public IP that may change after a router reboot or over time. DDNS handles this automatically.
